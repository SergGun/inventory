...
export default async function DashboardPage() {
  ...
  const productIsPresent = await prisma.product.count({
    where: { userId },
  });

  if (productIsPresent === 0) {
    await createDemoProducts(user.id);
  }
  ...
  const [zeroStockProducts, lowStockProducts, highStockProducts] =
    await Promise.all([
      prisma.product.findMany({
      ...
      prisma.product.findMany({
        where: { userId, quantity: { gt: 0, lte: 5 } },
        orderBy: { createdAt: 'desc' },
        take: 2,
      }),
      prisma.product.findMany({
